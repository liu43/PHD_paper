...
本章给出一种构造方法：使用 $\mL$、$\mL$ 顶点向量以及给定厚度 $\epsilon$，构建满足条件的壳 $\cS$。
...
\paragraph{引理证明}
\tr{依据 文献~\cite{Knabner2001TheIO}，(1)(2) 等价。先证明 (3) 的必要性。相邻曲面片相切意味着 $\frac{\partial f}{\partial u}$ 与 $\frac{\partial f}{\partial v}$ 与立柱方向 $\frac{\partial f}{\partial w}$ 共面，从而有 $J(p)=0$，与双射条件矛盾。相交情形同理。}

\tr{再证明 (3) 的充分性。设立柱上存在点 $p$ 使 $J(p)=0$。则 $\frac{\partial f}{\partial u}$、$\frac{\partial f}{\partial v}$ 与 $\frac{\partial f}{\partial w}$ 共面。若 $\frac{\partial f}{\partial u}$ 和 $\frac{\partial f}{\partial v}$ 分居 $\frac{\partial f}{\partial w}$ 两侧，则立柱与对侧曲面相交；若二者位于同侧，则相邻曲面片相切。}
...
\begin{figure*}[!t]
 ...
 右图展示每一轮中边塌缩(Collapse)、棱柱几何优化(Smooth)、边翻转(Flip)和棱柱缩放(Zoom)操作的次数。
 同时给出了九个渐进的壳结果。
...
这里仅使用中层 \Bezier 三角面 $\mt_\text{middle} \in \mM$ 评估 $E_{\text{quality}}$，因为按本章的构造方法，上层和下层与中层形态相近。
%
%\begin{equation}
...
\subsection{变体}
\paragraph{奇异点}
\tr{当两条以上棱线汇聚时会形成奇异点，即 $\mI$ 上不满足约束 (2) 的顶点。与文献~\cite{Jiang2020BijectivePI} 类似，我们扩展定义以允许这些奇异点处厚度为零。由此在存在奇异点时，棱柱结构会退化：一个奇异点时退化为棱锥，两个奇异点时退化为四面体，这两种情形都自然满足条件 (2)。}
\paragraph{含自交输入}
\tr{无自交要求是保证任意两张满足命题(~\ref{prop:bijective}) 的网格之间存在双射的必要条件。若放弃该双射性要求，该方法可扩展以处理含自交网格。}
	
\tr{若 $\mI$ 存在自交，本章算法可直接调整为生成保持局部单射的壳。此时映射在沉浸意义下仍为双射。为此需修改约束 (1)：将全局相交检测替换为局部三角片重叠检测。}
...
本章方法在高阶壳内部对输入网格进行重网格化（左中），得到高质量网格（左下）。
  随后在高质量网格上用热核法 计算距离场（下中），并利用双射投影将其传回输入网格（右下）。投影后的距离场更接近精确结果。
	}
	\label{fig:proxy}
\end{figure}
...
与文献~\cite{Jiang2020BijectivePI} 类似，本章展示四类应用，如图~\ref{fig:proxy}、\ref{fig:boolean}、\ref{fig:Dis_map} 和 \ref{fig:app_vector} 所示。

\tr{此外，本章高阶壳也支持高阶单元仿真。由于高阶壳的中层表面是 Bezier 三角网格，可采用文献~\cite{Khanteimouri20233Dbezier} 的算法生成用于仿真的高阶四面体网格，并利用壳内双射映射将属性传回。}
...
\begin{figure}[t]
	...
	\vspace{0mm}
	\caption{
本章方法在高阶壳内对两个网格并集进行重网格化，同时保持双射对应关系，颜色传递结果对此进行了可视化展示。
	}
	\label{fig:boolean}
\end{figure}
...
本章方法以 C++ 实现，所有实验在台式机上完成，硬件配置为 4.00 GHz Intel Core i7-4790K 与 16 GB 内存。
%
本算法唯一的用户可控参数是壳的目标厚度。
...
在耗时方面，本章方法不具备与线性壳构造~\cite{Jiang2020BijectivePI} 的竞争力。
但本章方法在高阶壳中获得了光滑双射投影，这对多类应用有帮助（图~\ref{fig:proxy}、\ref{fig:boolean} 和~\ref{fig:Dis_map}）。
...
%\todo{shell space}
如图~\ref{fig:Space} 所示，我们与 线性壳 在壳空间均匀性上进行比较。输入网格到本章高阶壳的距离分布更均匀。
%
...
若源网格属性平滑且对应目标网格平滑，本章方法的投影可得到平滑的属性像。
...
本章围绕壳空间内的光滑双射投影问题，提出了一种高阶壳表示及其实用构造算法。我们以高阶三角棱柱为基本单元，在壳内构造连续向量场，并据此定义两张曲面之间的双射投影。围绕这一目标，本章给出了 \Bezier 三角面与双线性侧面的几何构造方式、壳内投影的可判定双射条件，以及基于边塌缩、几何优化、边翻转和棱柱缩放的迭代构造流程。实验结果表明，该方法能够在 8300 余个模型上稳定生成高阶壳，并在重网格化属性回传、布尔相关应用与位移贴图等任务中取得良好效果。与线性壳相比，本章方法得到的投影更光滑，壳空间分布也更均匀。
