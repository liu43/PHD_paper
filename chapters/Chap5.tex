...
本章据此构造笼空间中的半解析半数值的高阶双调和坐标。方法上，我们在高阶边界元框架下，将曲边笼边界、边界条件以及未知场统一表示为 Bernstein 多项式，并结合解析积分与数值求解建立可计算的坐标形式。由此得到的坐标以边界控制与低扭曲为核心目标：一方面，它能够更好地约束边界值及其导数，从而提高曲边笼编辑中的边界可控性；另一方面，它在内部保持较平滑的传播过程，以减轻剪切、拉伸与局部伪影。\S~\ref{sec:bihc-method} 节首先回顾双调和坐标的边界元求解基础，随后给出高阶单元下的边界积分方程重写、代数系统求解与变形控制方法；\S~\ref{sec:bihc-results} 节通过时间、不同目标次数以及与现有坐标方法的对比实验，评估所提出方法在计算效率、边界控制能力与变形质量上的表现。
...
%
在代数方程组求解方面，文献\cite{Weber2012} 将代数约束~\eqref{equ:bihc-BIE2} 代入~\eqref{equ:bihc-BIE1}，形成线性系统并求解。
在三维情形中，文献\cite{thiery2024biharmonic} 同时最小化两类约束，从而得到更平稳的变形结果。高阶边界元设置的整体示意见图~\ref{fig:bihc-schematic}。
...
因此，只需计算 \(F_{h,2}^{c,\eta}\) 与 \(F_{h,4}^{c,\eta}\)。文献\cite{liu2024polygc} 给出了如下引理：
\begin{lemma}
...
%
此时，上述积分求值涉及奇异积分计算。关于该问题，在边界元方法中已有大量研究~\cite{Fischer1999ApplicationOH}。我们遵循文献~\cite{Weber2012} 的建议：当 \(\eta\) 趋近边界时，这些积分的极限存在。
\tr{在本问题中，上述解析方法无法直接计算该极限，因为每一项 \(F_{h,2}^{c,\eta}\) 都趋于无穷大。我们在补充推导中单独给出该极限的显式表达。另一种实用策略是将点 \(\eta^{\text{b}}\) 略微向笼内部偏移，再直接计算积分。实验表明，这一做法同样可得到准确结果。}
...
此时，方程组中的 8 个矩阵可简化为 5 个。由此可得到本章正则化双调和坐标的表达：
...
\(\alpha(\eta)\) 与 \(\beta(\eta)\) 共同构成本章双调和坐标。
...
在这种情况下，需要考虑笼的全局变形。
按照文献~\cite{Weber2012} 与文献~\cite{thiery2024biharmonic} 的做法，我们给出两种根据边界位置自动推导边界导数的方法。
...
\tr{
本章方法采用 C++ 实现，使用 Eigen~\cite{eigenweb} 求解线性系统。全部实验在一台配备 4.00 GHz Intel Core i7-4790K 处理器和 16 GB 内存的台式机上完成。
}

\paragraph{时间与内存}
\tr{本章展示的所有基于笼变形算法均可实时运行。
...
相比之下，图~\ref{fig:bihc-teaser} 中的章鱼案例接近最复杂场景，笼由 22 条三次 \Bezier 曲线组成。步骤 (1) 耗时 1.02 s，其中 96.6\% 时间用于 Eigen 库中的 6 次矩阵运算；步骤 (2) 对 8800 个点耗时 0.40 s。在本章所有案例中，预处理总内存占用均未超过 150 MB。}
...
尽管本章方法允许变形后笼具有任意高次数，但需注意：次数越高，积分计算 \eqref{equ:bihc-basis_inte} 越复杂，求解坐标 \eqref{equ:bihc-final_BHC} 所需矩阵约束也越大，从而带来更高预计算时间。
...
\paragraph{对比}
当初始笼为线性时，我们选择 Cubic MVC~\cite{Li2013} 与 Polynomial Green Coordinate（PolyGC）~\cite{MichelThiery2023} 作为对比方法；当初始笼为高阶时，我们选择文献~\cite{liu2024polygc} 与文献~\cite{Lin2024PolynomialCC} 进行对比。

与本章坐标一样，Cubic MVC 同时插值边界上的函数值与导数。但该坐标与常规均值坐标一样会产生伪影，尤其在非凸形状上表现较差（见图~\ref{fig:bihc-cmp-cmvc}）。此外，本章坐标继承了调和坐标的优势，即最小化所谓 Hessian 能量~\cite{Stein2018NaturalBC}，并具有调和拉普拉斯，从而产生更低失真的变形（见图~\ref{fig:bihc-more-cmp}）。同时，Cubic MVC 不支持高阶笼。

文献\cite{Lin2024PolynomialCC} 通过将多项式 Cauchy 坐标与其逆映射结合，实现了曲边笼之间的变形。然而，其方法需要用户额外指定一个中间线性笼，且逆映射计算依赖数值积分，不具解析表达。
\tr{此外，他们的高阶笼需要后验调整，无法像本章方法那样紧密贴合变形后形状。
这些原因和差异使我们无法与其进行公平对比。}

本章坐标在表达形式和变形结果上与 PolyGC 和 CurvedGC 更为接近。
PolyGC 与 CurvedGC 用插值性换取保角性；而本章坐标提供了在保角性与插值性之间灵活折中的方式。
...
\section{本章小结}
在本章中，针对第五章高阶柯西坐标在共形性与边界控制之间难以兼顾的问题，我们构造了适用于曲边笼的半解析半数值高阶双调合坐标。首先，我们回顾了双调和边值问题及其边界元求解基础；随后，在高阶边界元框架下，将曲边笼边界、边界条件与未知场统一表示为 Bernstein 多项式，建立了高阶单元上的积分计算、矩阵约束与坐标求解方法；进一步地，我们讨论了精度提升、变形能量以及不同权重下的边界控制策略。最后，通过时间与内存、不同目标次数以及与现有坐标方法的对比实验，验证了本章方法能够在增强边界可控性的同时保持较小的内部扭曲，并在保角性与插值性之间提供灵活的折中。
