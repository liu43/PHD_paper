...
\paragraph{基于笼的变形}
本文在图~\ref{fig:cau-more-example} 中展示了多组图像变形示例。
本文坐标能够对有机形状和机械形状提供光滑且保角的变形结果。
本文方法也支持对曲线段多项式次数进行升阶和降阶（图~\ref{fig:cau-diff-order}）。
...
为支持由把手驱动的变形，本文利用坐标导数构建目标函数并进行优化求解，这与文献 \cite{Weber2009} 和 文献\cite{Lin2024PolynomialCC} 的做法类似。
目标函数包含把手点之间距离平方和，以及一个由边界上坐标二阶导数的 $\ell_2$ 范数组成的光滑项。
%
该优化问题是最小二乘问题。
通过线性求解得到最优解。
此外，在线性系统中，拖动把手时系数矩阵保持不变；因此本文在预处理阶段仅做一次预分解，从而支持交互式把手编辑（图~\ref{fig:cau-teaser}、\ref{fig:cau-more-example} 和 \ref{fig:cau-diff-order}）。
...
\begin{figure}[t]
	...
	\vspace{0mm}
	\caption{
		利用本文坐标及其导数在高阶输入笼上实现的多组基于笼变形与点到点变形结果。
	}
	...
	\begin{overpic}[width=0.99\linewidth]{figures/chap4_cage_complex.pdf}
		{
			...
			\put(64,-1.5){\small 本文坐标的 P2P}
			\put(82,-1.5){\small 本文的笼变形}
		}
	\end{overpic}
    \vspace{2mm}
	\caption{
...
%
由于线性笼是高阶笼的特例，本文坐标也可用于直边笼并实现保角变形。
%
本文通过为线性输入笼的每条边指定不同的非线性参数化，获得不同的保角变形结果（图~\ref{fig:cau-cmp-mvc}）。
...
%
为公平比较，多项式 Cauchy 坐标与本文方法使用相同数量的控制点。
...
%
最后，在图~\ref{fig:cau-comparep2p_poly} 中，本文比较了在输入笼自由度相同条件下的 P2P 变形。
%
